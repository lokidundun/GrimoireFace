...
import { importBuiltinCategoryFile, loadAllBuiltinModulesParallel } from '@/lib/questionLoader'
import {
  AI_PROVIDER_PRESETS,
  type AIConfig,
  type AIProviderId,
...
watch(() => props.open, (open) => {
  if (open) {
    ...
    getBuiltinCategories().then((c) => { builtinCategories.value = c })
...
function toggleApiKeyInput(providerId: string) {
  showApiKeyInput.value = { ...showApiKeyInput.value, [providerId]: !showApiKeyInput.value[providerId] }
}
...
function handleSelectModel(providerId: AIProviderId, modelValue: string) {
  ...
  showToast(`已切换到 ${getAIProviderPreset(providerId).shortLabel} · ${modelValue}`)
}
...
async function handleTest() {
  ...
  const apiKey = aiStore.getProviderApiKey(provider)
  const preset = getAIProviderPreset(provider)
  if (!apiKey) { testResult.value = { ok: false, message: `请先在设置中为 ${preset.label} 配置 API Key` }; return }

  testing.value = true
  testResult.value = null
  try {
    const response = await fetch(buildChatCompletionsUrl(preset.baseUrl), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model: aiStore.config.model, messages: [{ role: 'user', content: 'Hi, reply with exactly: OK' }], max_tokens: 10, stream: false }),
    })
    if (!response.ok) {
...
async function handleMoveModule(moduleName: string, fromCategory: string, toCategory: string) {
...
async function handleAddBuiltinCategory() {
  ...
  const map = await getCategoryMap()
  if (!map[name]) {
    map[name] = { name, modules: [], builtin: false, order: Object.keys(map).length }
    await saveCategoryMap(map)
  }
...
async function handleConfirmEditBuiltinCategory(oldName: string) {
  ...
  if (name !== oldName) {
    const map = await getCategoryMap()
    if (map[oldName] && !map[oldName].builtin) {
...
async function handleDeleteBuiltinCategory(name: string) {
  if (!confirm(`确定删除分类「${name}」？对应的题目和分类映射将被移除。`)) return
  const current = await getBuiltinCategories()
...
async function handleRemoveBuiltinFile(categoryName: string, fileIndex: number) {
  const current = await getBuiltinCategories()
...
async function handleAddBuiltinFile(categoryName: string) {
  const newFile = prompt(`为「${categoryName}」添加新的 JSON 文件路径：`, 'newcategory/file.json')
...
async function handleUploadBuiltinFile(categoryName: string, e: Event) {
  ...
  try {
    ...
    console.log('[Upload]', file.name, 'loaded:', result.loaded, 'errors:', result.errors)
    if (result.loaded > 0) {
      const current = await getBuiltinCategories()
      const cat = current.find((c) => c.category === categoryName)
...
const categoryEntries = computed(() =>
  Object.values(categoryMap.value)
    .sort((a, b) => {
    ...
    >
      ...
      >
        <button
          ...
          :title="{ auth: '认证管理', model: '模型选择', ai: 'AI 助手', study: '刷题偏好', data: '数据管理', sync: '云同步' }[t]"
        >
          <!-- Auth icon -->
          <svg v-if="t === 'auth'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
          </svg>
          <!-- Model icon -->
          <svg v-else-if="t === 'model'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0">
            <circle cx="12" cy="12" r="3"/><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/>
          </svg>
          <!-- AI icon -->
          <svg v-else-if="t === 'ai'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0">
          ...
          <span class="settings-tab-label">{{ { auth: '认证', model: '模型', ai: 'AI', study: '刷题', data: '数据', sync: '同步' }[t] }}</span>
        </button>
      </div>
      ...
      >
        ...
        <template v-if="tab === 'auth'">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px">
            ...
            <span style="font-size: 13px; font-weight: 600; color: var(--text)">认证管理</span>
          </div>
          <p style="font-size: 11px; color: var(--text-3); line-height: 1.5">为每个 AI 提供者配置 API Key，配置完成后即可在模型页选择对应模型。</p>

          <!-- Search box -->
          <div style="position: relative">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: var(--text-3)">
            ...
            <input
              ...
              type="text"
              placeholder="搜索供应商..."
              style="width: 100%; padding: 8px 12px 8px 32px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box"
            />
            <button
              v-if="providerSearchQuery"
              type="button"
              @click="providerSearchQuery = ''"
              style="position: absolute; right: 8px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-3); cursor: pointer; padding: 2px; display: flex; align-items: center"
            >
          ...
          <div style="padding: 12px; border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle); display: flex; flex-direction: column; gap: 10px">
            <span style="font-size: 13px; font-weight: 600; color: var(--text)">自定义兼容接口</span>
            <input :value="localConfig.baseUrl" @input="patch({ baseUrl: ($event.target as HTMLInputElement).value })" placeholder="Base URL" style="width: 100%; padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box" />
            <input :value="localConfig.model" @input="patch({ model: ($event.target as HTMLInputElement).value })" placeholder="模型名称" style="width: 100%; padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box" />
            <template v-if="showApiKeyInput['custom']">
              <input v-model="apiKeyInputs['custom']" type="password" placeholder="API Key" @keyup.enter="submitApiKey('custom')" style="width: 100%; padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box" autocomplete="off" spellcheck="false" />
              <div style="display: flex; gap: 8px; justify-content: flex-end">
                <button type="button" @click="toggleApiKeyInput('custom')" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: transparent; color: var(--text-2); font-size: 12px; cursor: pointer">取消</button>
                <button type="button" @click="submitApiKey('custom')" style="padding: 5px 10px; border-radius: 7px; border: none; background: var(--primary); color: white; font-size: 12px; cursor: pointer">保存</button>
              </div>
            </template>
            <div v-else style="display: flex; gap: 8px">
              <button v-if="!aiStore.isProviderAuthenticated('custom')" type="button" @click="toggleApiKeyInput('custom')" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: var(--surface); color: var(--text-2); font-size: 12px; cursor: pointer">配置 API Key</button>
              <button v-else type="button" @click="handleRemoveAuth('custom')" style="padding: 5px 10px; border-radius: 7px; border: 1px solid rgba(239,68,68,0.2); background: transparent; color: var(--danger); font-size: 12px; cursor: pointer">删除认证</button>
            </div>
          </div>

          <!-- Provider list -->
          <div v-if="filteredProviders.length === 0" style="text-align: center; padding: 24px; color: var(--text-3); font-size: 13px">
            未找到匹配的供应商
          </div>
          <div v-for="preset in filteredProviders" :key="preset.id" style="padding: 12px; border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle); display: flex; flex-direction: column; gap: 10px">
            <div style="display: flex; align-items: center; justify-content: space-between">
              <div style="display: flex; align-items: center; gap: 8px">
                <span style="font-size: 13px; font-weight: 600; color: var(--text)">{{ preset.label }}</span>
                <span :style="{ fontSize: '11px', padding: '2px 7px', borderRadius: '5px', fontWeight: 500, background: aiStore.isProviderAuthenticated(preset.id) ? 'var(--success-light)' : 'var(--surface-3)', color: aiStore.isProviderAuthenticated(preset.id) ? 'var(--success)' : 'var(--text-3)' }"
                >{{ aiStore.isProviderAuthenticated(preset.id) ? '已认证' : '未认证' }}</span>
              </div>
            </div>
            ...
            <template v-if="showApiKeyInput[preset.id]">
              <input
                ...
                style="width: 100%; padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box"
                autocomplete="off"
                spellcheck="false"
              />
              <div style="display: flex; gap: 8px; justify-content: flex-end">
                <button type="button" @click="toggleApiKeyInput(preset.id)" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: transparent; color: var(--text-2); font-size: 12px; cursor: pointer">取消</button>
                <button type="button" @click="submitApiKey(preset.id)" style="padding: 5px 10px; border-radius: 7px; border: none; background: var(--primary); color: white; font-size: 12px; cursor: pointer">保存</button>
              </div>
            </template>
            <template v-else>
              <div style="display: flex; gap: 8px">
                <button v-if="!aiStore.isProviderAuthenticated(preset.id)" type="button" @click="toggleApiKeyInput(preset.id)" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: var(--surface); color: var(--text-2); font-size: 12px; cursor: pointer">配置 API Key</button>
                <button v-else type="button" @click="handleRemoveAuth(preset.id)" style="padding: 5px 10px; border-radius: 7px; border: 1px solid rgba(239,68,68,0.2); background: transparent; color: var(--danger); font-size: 12px; cursor: pointer">删除认证</button>
              </div>
            </template>
          </div>
        </template>
        ...
        <template v-if="tab === 'model'">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px">
            ...
            <span style="font-size: 13px; font-weight: 600; color: var(--text)">模型选择</span>
          </div>
          <p style="font-size: 11px; color: var(--text-3); line-height: 1.5">选择当前使用的 AI 模型。未认证的提供者模型不可选，请先前往认证页配置 API Key。</p>

          <div v-for="preset in AI_PROVIDER_PRESETS.filter(p => p.id !== 'custom')" :key="preset.id" style="display: flex; flex-direction: column; gap: 8px">
            <div style="display: flex; align-items: center; gap: 8px">
              <span style="font-size: 12px; font-weight: 600; color: var(--text-2)">{{ preset.label }}</span>
              <span :style="{ fontSize: '11px', padding: '2px 7px', borderRadius: '5px', fontWeight: 500, background: aiStore.isProviderAuthenticated(preset.id) ? 'var(--success-light)' : 'var(--surface-3)', color: aiStore.isProviderAuthenticated(preset.id) ? 'var(--success)' : 'var(--text-3)' }"
              >{{ aiStore.isProviderAuthenticated(preset.id) ? '已认证' : '未认证' }}</span>
            </div>
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px">
              <button
                v-for="m in preset.models" :key="m.value"
                type="button"
                @click="aiStore.isProviderAuthenticated(preset.id) && handleSelectModel(preset.id, m.value)"
                ...
                "
                :style="{
                  borderColor: aiStore.config.provider === preset.id && aiStore.config.model === m.value ? 'var(--primary)' : 'var(--border)',
                  background: aiStore.config.provider === preset.id && aiStore.config.model === m.value ? 'var(--primary-light)' : 'var(--surface)',
                  color: aiStore.config.provider === preset.id && aiStore.config.model === m.value ? 'var(--primary)' : 'var(--text-2)',
                  opacity: aiStore.isProviderAuthenticated(preset.id) ? 1 : 0.5,
                  cursor: aiStore.isProviderAuthenticated(preset.id) ? 'pointer' : 'not-allowed',
                }"
              >
                <div style="width: 14px; height: 14px; border-radius: 50%; border: 2px solid; flex-shrink: 0; display: flex; align-items: center; justify-content: center"
                  :style="{ borderColor: aiStore.config.provider === preset.id && aiStore.config.model === m.value ? 'var(--primary)' : 'var(--border)' }"
                >
                  <div v-if="aiStore.config.provider === preset.id && aiStore.config.model === m.value" style="width: 7px; height: 7px; border-radius: 50%; background: var(--primary)" />
                </div>
          ...
          <div style="display: flex; flex-direction: column; gap: 8px">
            <div style="display: flex; align-items: center; gap: 8px">
              <span style="font-size: 12px; font-weight: 600; color: var(--text-2)">自定义兼容接口</span>
              <span :style="{ fontSize: '11px', padding: '2px 7px', borderRadius: '5px', fontWeight: 500, background: aiStore.isProviderAuthenticated('custom') ? 'var(--success-light)' : 'var(--surface-3)', color: aiStore.isProviderAuthenticated('custom') ? 'var(--success)' : 'var(--text-3)' }"
              >{{ aiStore.isProviderAuthenticated('custom') ? '已认证' : '未认证' }}</span>
            </div>
            <button
              type="button"
              @click="aiStore.isProviderAuthenticated('custom') && handleSelectModel('custom', aiStore.config.model)"
              ...
              "
              :style="{
                borderColor: aiStore.config.provider === 'custom' ? 'var(--primary)' : 'var(--border)',
                background: aiStore.config.provider === 'custom' ? 'var(--primary-light)' : 'var(--surface)',
                color: aiStore.config.provider === 'custom' ? 'var(--primary)' : 'var(--text-2)',
                opacity: aiStore.isProviderAuthenticated('custom') ? 1 : 0.5,
                cursor: aiStore.isProviderAuthenticated('custom') ? 'pointer' : 'not-allowed',
              }"
            >
              <div style="width: 14px; height: 14px; border-radius: 50%; border: 2px solid; flex-shrink: 0; display: flex; align-items: center; justify-content: center"
                :style="{ borderColor: aiStore.config.provider === 'custom' ? 'var(--primary)' : 'var(--border)' }"
              >
                <div v-if="aiStore.config.provider === 'custom'" style="width: 7px; height: 7px; border-radius: 50%; background: var(--primary)" />
              </div>
              <span style="font-weight: 500">{{ aiStore.config.model || '未设置模型' }}</span>
            </button>
          </div>
        </template>
        ...
        <template v-if="tab === 'ai'">
          <div style="display: flex; align-items: center; justify-content: space-between">
            <span style="font-size: 13px; font-weight: 600; color: var(--text)">启用 AI 功能</span>
            <button
              type="button"
              @click="patch({ enabled: !localConfig.enabled })"
              :style="{
                width: 40, height: 22, borderRadius: 11, border: 'none',
                background: localConfig.enabled ? 'var(--primary)' : 'var(--border)',
                cursor: 'pointer', position: 'relative', transition: 'background 0.2s',
              }"
            >
              <div :style="{ position: 'absolute', top: 2, left: localConfig.enabled ? 20 : 2, width: 18, height: 18, borderRadius: '50%', background: 'white', transition: 'left 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.15)' }" />
            </button>
          </div>
        ...
        <template v-if="tab === 'study'">
          ...
          <div style="display: flex; flex-direction: column; gap: 8px">
            <div style="display: flex; align-items: center; justify-content: space-between">
              <div style="font-size: 12px; font-weight: 500; color: var(--text-2)">题库分类管理</div>
              <button
                type="button"
                @click="addingCategory = true"
                style="font-size: 11px; color: var(--primary); background: none; border: none; cursor: pointer; padding: 2px 6px; display: flex; align-items: center; gap: 2px"
              >
            ...
            <!-- Add category form -->
            <div v-if="addingCategory" style="display: flex; flex-direction: column; gap: 6px; padding: 10px; border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle)">
              <input
              ...
              />
              <div style="display: flex; gap: 8px; justify-content: flex-end">
                <button type="button" @click="addingCategory = false; newCategoryName = ''" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: transparent; color: var(--text-2); font-size: 12px; cursor: pointer">取消</button>
                <button type="button" @click="handleAddCategory" style="padding: 5px 10px; border-radius: 7px; border: none; background: var(--primary); color: white; font-size: 12px; cursor: pointer">确认</button>
              </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 6px">
              <div
                ...
                style="border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle); overflow: hidden"
              >
                <!-- Category header -->
                <div
                ...
                >
                  <div style="display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0">
                    <svg
                      ...
                      :style="{ transform: expandedCategories.has(cat.name) ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s', color: 'var(--text-3)', flexShrink: 0 }"
                    >
                    ...
                    <template v-if="editingCategory === cat.name">
                      <input
                        ...
                        @click.stop
                        style="flex: 1; padding: 4px 8px; border-radius: 6px; border: 1px solid var(--primary); background: var(--surface); color: var(--text); font-size: 13px; outline: none; min-width: 0"
                      />
                    </template>
                    <template v-else>
                      <span style="font-size: 13px; font-weight: 500; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis">{{ cat.name }}</span>
                    </template>

                    <span style="font-size: 11px; color: var(--text-3); flex-shrink: 0">{{ cat.modules.length }} 个模块</span>
                  </div>
                  ...
                  <div style="display: flex; align-items: center; gap: 2px; flex-shrink: 0">
                    ...
                    <button
                      type="button"
                      @click.stop="studyStore.toggleCategoryVisibility(cat.name)"
                      :style="{ padding: '4px 6px', borderRadius: '6px', border: 'none', background: 'none', color: studyStore.hiddenCategories.has(cat.name) ? 'var(--text-3)' : 'var(--primary)', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center' }"
                      :title="studyStore.hiddenCategories.has(cat.name) ? '已隐藏，点击展示' : '已展示，点击隐藏'"
                    >
                      <svg v-if="!studyStore.hiddenCategories.has(cat.name)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    ...
                    <button
                      ...
                      style="padding: 4px 6px; border-radius: 6px; border: none; background: none; color: var(--text-3); cursor: pointer; font-size: 11px"
                      title="编辑名称"
                    >
                    ...
                    <button
                      ...
                      style="padding: 4px 6px; border-radius: 6px; border: none; background: none; color: var(--primary); cursor: pointer; font-size: 11px"
                      title="确认"
                    >
                ...
                <div v-if="expandedCategories.has(cat.name)" style="border-top: 1px solid var(--border-subtle); padding: 8px 12px 10px 32px; display: flex; flex-direction: column; gap: 6px">
                  <div v-if="cat.modules.length === 0" style="font-size: 12px; color: var(--text-3); padding: 4px 0">
                    暂无模块
                  </div>
                  <div
                    v-for="mod in cat.modules" :key="mod"
                    style="display: flex; align-items: center; justify-content: space-between; gap: 8px"
                  >
                    <span style="font-size: 12px; color: var(--text-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis">{{ mod }}</span>
                    <select
                      @change="handleMoveModule(mod, cat.name, ($event.target as HTMLSelectElement).value); ($event.target as HTMLSelectElement).value = ''"
                      style="padding: 3px 6px; border-radius: 5px; border: 1px solid var(--border); background: var(--surface); color: var(--text-2); font-size: 11px; cursor: pointer; outline: none; flex-shrink: 0"
                    >
                      <option value="" disabled selected>移动到...</option>
                      <option v-for="target in categoryEntries.filter(c => c.name !== cat.name)" :key="target.name" :value="target.name">{{ target.name }}</option>
                    </select>
                  </div>
                </div>
              </div>
          ...
          <div style="display: flex; flex-direction: column; gap: 8px">
            <div style="display: flex; align-items: center; justify-content: space-between">
              <div style="font-size: 12px; font-weight: 500; color: var(--text-2)">内置题库分类配置</div>
              <div style="display: flex; align-items: center; gap: 6px">
                ...
                <button
                  ...
                  style="font-size: 11px; color: var(--primary); background: none; border: 1px solid var(--primary); border-radius: 6px; cursor: pointer; padding: 2px 8px; display: flex; align-items: center; gap: 2px"
                >
            ...
            <div v-if="addingBuiltinCategory" style="display: flex; flex-direction: column; gap: 6px; padding: 10px; border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle)">
              <input
              ...
              <textarea
                ...
                rows="4"
                style="width: 100%; padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 13px; outline: none; box-sizing: border-box; resize: vertical; font-family: inherit"
              />
              <div style="display: flex; gap: 8px; justify-content: flex-end">
                <button type="button" @click="addingBuiltinCategory = false; newBuiltinCategoryName = ''; newBuiltinCategoryFiles = ''" style="padding: 5px 10px; border-radius: 7px; border: 1px solid var(--border); background: transparent; color: var(--text-2); font-size: 12px; cursor: pointer">取消</button>
                <button type="button" @click="handleAddBuiltinCategory" style="padding: 5px 10px; border-radius: 7px; border: none; background: var(--primary); color: white; font-size: 12px; cursor: pointer">确认</button>
              </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 6px">
              <div
                v-for="cat in builtinCategories" :key="cat.category"
                style="border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle); overflow: hidden"
              >
                <!-- Category header -->
                <div
                ...
                >
                  <div style="display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0">
                    <svg
                      width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                      :style="{ transform: expandedBuiltinCategories.has(cat.category) ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s', color: 'var(--text-3)', flexShrink: 0 }"
                    >
                      <polyline points="9 18 15 12 9 6" />
                    </svg>
                    ...
                    <template v-if="editingBuiltinCategory === cat.category">
                      <input
                        ...
                        type="text"
                        @keyup.enter="handleConfirmEditBuiltinCategory(cat.category)"
                        @click.stop
                        style="flex: 1; padding: 4px 8px; border-radius: 6px; border: 1px solid var(--primary); background: var(--surface); color: var(--text); font-size: 13px; outline: none; min-width: 0"
                      />
                    </template>
                    <template v-else>
                      <span style="font-size: 13px; font-weight: 500; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis">{{ cat.category }}</span>
                    </template>

                    <span style="font-size: 11px; color: var(--text-3); flex-shrink: 0">{{ cat.files.length }} 个文件</span>
                  </div>
                  ...
                  <div style="display: flex; align-items: center; gap: 2px; flex-shrink: 0">
                    ...
                    <button
                      v-if="editingBuiltinCategory === cat.category"
                      type="button"
                      @click.stop="handleConfirmEditBuiltinCategory(cat.category)"
                      style="padding: 4px 6px; border-radius: 6px; border: none; background: none; color: var(--primary); cursor: pointer; font-size: 11px"
                    >
                    ...
                    <button
                      ...
                      style="padding: 4px 6px; border-radius: 6px; border: none; background: none; color: var(--danger); cursor: pointer; font-size: 11px"
                    >
                ...
                <!-- Expanded file list -->
                <div v-if="expandedBuiltinCategories.has(cat.category)" style="padding: 0 12px 10px 36px; display: flex; flex-direction: column; gap: 4px">
                  <!-- File list in edit mode -->
                  <template v-if="editingBuiltinCategory === cat.category">
                    <div
                      v-for="(file, idx) in editBuiltinFiles" :key="idx"
                      style="display: flex; align-items: center; gap: 6px"
                    >
                      <input
                        v-model="editBuiltinFiles[idx]"
                        type="text"
                        style="flex: 1; padding: 4px 8px; border-radius: 5px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 12px; outline: none"
                      />
                      <button
                        type="button"
                        @click="editBuiltinFiles.splice(idx, 1)"
                        style="padding: 2px 4px; border-radius: 4px; border: none; background: none; color: var(--danger); cursor: pointer; font-size: 11px"
                      >
                    ...
                    <button
                      ...
                      style="padding: 4px 8px; border-radius: 6px; border: 1px dashed var(--border); background: none; color: var(--text-3); cursor: pointer; font-size: 11px; display: flex; align-items: center; gap: 4px; align-self: flex-start"
                    >
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                      添加文件路径
                    </button>
                  ...
                  <template v-else>
                    <div
                      v-for="(file, idx) in cat.files" :key="file"
                      style="display: flex; align-items: center; justify-content: space-between; gap: 6px"
                    >
                      <code style="font-size: 11px; color: var(--text-2); background: var(--surface); padding: 2px 6px; border-radius: 4px; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap">{{ file }}</code>
                      <button
                        type="button"
                        @click="handleRemoveBuiltinFile(cat.category, idx)"
                        style="padding: 2px 4px; border-radius: 4px; border: none; background: none; color: var(--danger); cursor: pointer; font-size: 10px"
                      >
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      </button>
                    </div>
                    ...
                    <button
                      type="button"
                      @click="($event.currentTarget as HTMLElement).previousElementSibling?.click()"
                      style="padding: 4px 8px; border-radius: 6px; border: 1px dashed var(--primary); background: none; color: var(--primary); cursor: pointer; font-size: 11px; display: flex; align-items: center; gap: 4px; align-self: flex-start"
                    >
              ...
              <!-- Empty state -->
              <div v-if="builtinCategories.length === 0" style="padding: 20px; text-align: center; color: var(--text-3); font-size: 13px; border-radius: 10px; background: var(--surface-2); border: 1px solid var(--border-subtle)">
      ...
      <div style="border-top: 1px solid var(--border-subtle); flex-shrink: 0">
        ...
        <div v-if="tab === 'ai'" style="border-bottom: 1px solid var(--border-subtle)">
          ...
          <div style="padding: 12px 20px; display: flex; align-items: center; gap: 8px">
            <button type="button" @click="handleSaveAIConfig" :disabled="!isDirty"
              :style="{
            ...
            <button type="button" @click="handleResetAIConfig"
